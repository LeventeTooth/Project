...
@section('content')
    ...
    <div class="flex items-center justify-center min-h-[calc(100vh-260px)] bg-cover bg-center">
        <form class="bg-stone-300 p-8 rounded-lg shadow-lg space-y-6" method="POST" action="{{ route('auth.login') }}">
            @csrf
            @if(session('success'))
                <p class="bg-green-200 text-green-900 border-2 rounded border-green-900 text-center p-3">{{ session('success') }} <br> Profilom-nál lehet csapatot választani</p>
            </a>
            @endif
            <h2 class="text-2xl font-bold text-center text-stone-700">Bejelentkezés</h2>
